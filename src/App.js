...
import {
  ...
  getMondayOf, getWeekDays, DAY_SHORT, getActiveTask,
...
function DragGhost({ x, y, text, isCopy }) {
  if (!text) return null
  return (
    <div style={{position:'fixed',left:x+12,top:y-15,pointerEvents:'none',zIndex:999,
      background: isCopy ? 'rgba(79,247,162,.2)' : 'rgba(79,142,247,.25)',
      border: `2px dashed ${isCopy ? '#4ff7a2' : '#4f8ef7'}`,
      borderRadius:4,padding:'4px 10px',fontSize:11,
      color: isCopy ? '#4ff7a2' : '#4f8ef7',whiteSpace:'nowrap'}}>
      {text}
    </div>
  )
}
...
const OFFICE_COLORS = {Brisbane:'#4f8ef7',Chennai:'#f7a24f',Bangkok:'#4ff7a2'}
...
export default function App() {
  ...
  // Mouse move / up handlers on the grid header to resolve date columns
  function getDateAtX(clientX) {
    ...
    let dayIdx = 0
    ths.forEach((th, i) => {
      if (i === 0) return // name col
      const rect = th.getBoundingClientRect()
      // skip weekend merged cols (they have data-weekend attribute)
  ...
  useEffect(() => {
    function onMouseMove(e) {
      ...
      if (dragState.current) {
        const {entry, handle, currentStart, currentEnd} = dragState.current
        ...
        const s = parseLocalDate(dragState.current.currentStart)
        const t = parseLocalDate(dragState.current.currentEnd)
        const days = Math.round((t-s)/86400000)+1
        const lbl = handle==='end' ? `End → ${fmtDisplay(targetDay)}` : `Start → ${fmtDisplay(targetDay)}`
        setDragGhost(g => ({...g, text:`${entry.task}  |  ${lbl} (${days}d)`}))
      }
  ...
  }, [tasks, weekStart, showToast, assignments])

  // ── Row reorder ──
  const [rowDragSrc, setRowDragSrc] = useState(null)

  async function moveRow(memberId, office, dir) {
    const officeMembers = teamMembers.filter(m=>m.office===office).sort((a,b)=>a.sort_order-b.sort_order)
    const idx = officeMembers.findIndex(m=>m.id===memberId)
    const newIdx = idx+dir
    if (newIdx < 0 || newIdx >= officeMembers.length) return
    const a = officeMembers[idx], b = officeMembers[newIdx]
    await Promise.all([
  ...
  function handleRowDrop(targetId, targetOffice) {
    ...
    const officeMembers = teamMembers.filter(m=>m.office===targetOffice).sort((a,b)=>a.sort_order-b.sort_order)
    const srcIdx = officeMembers.findIndex(m=>m.id===rowDragSrc.id)
    const tgtIdx = officeMembers.findIndex(m=>m.id===targetId)
    if (srcIdx<0||tgtIdx<0) { setRowDragSrc(null); return }
    const reordered = [...officeMembers]
    const [moved] = reordered.splice(srcIdx, 1)
    reordered.splice(tgtIdx, 0, moved)
    Promise.all(reordered.map((m,i) => supabase.from('team_members').update({sort_order:i+1}).eq('id',m.id)))
  ...
  return (
    <div style={{background:'#0f1117',minHeight:'100vh',color:'#e2e8ff',fontFamily:'DM Mono,monospace'}}>
      ...
      {assignModal && <AssignModal modal={assignModal} onClose={()=>setAssignModal(null)}
        ...
        onSave={saveTask} onClear={clearTask} showToast={showToast} />}
...
function WorkloadTab({ days, week1Work, week2Work, allWorkdays, weekStart, setWeekStart,
  ...
  setLeaveModal, setPhModal, startResize, startCopy,
  rowDragSrc, setRowDragSrc, moveRow, handleRowDrop, showToast }) {
        ...
        <table style={{width:'100%',minWidth:900,borderCollapse:'collapse',tableLayout:'fixed'}}>
          ...
          <thead id="grid-thead">
            <tr>
              ...
              <th data-weekend="1" style={{...thStyle,background:'#0d1018',opacity:.5,fontSize:8,
                color:'#5a6380',writingMode:'vertical-rl'}}>S/S</th>
              {week2Work.map((d,i)=>(
                <th key={i} style={thStyle}>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:16}}>
        {['Brisbane','Chennai','Bangkok'].map(o=>(
          <LeavePanel key={o} office={o} color={OFFICE_COLORS[o]}
            items={leaveByOffice[o]||[]}
            onAdd={()=>setLeaveModal({office:o})}
...
const tdStyle = {background:'#181c27',border:'1px solid #2a3050',verticalAlign:'top',
...
function MemberRow({ member, week1Work, week2Work, getActive, projects, adminTasks,
  setAssignModal, startResize, startCopy, rowDragSrc, setRowDragSrc, onMoveRow, onRowDrop }) {
  ...
  function renderWeek(workDays) {
    ...
    while (i < workDays.length) {
      ...
      while(j<workDays.length) {
        const nextA = getActive(member.name, fmtDate(workDays[j]))
        if(nextA && nextA.startDs===startDs && JSON.stringify(nextA.entry)===JSON.stringify(entry)) { span++;j++ }
        else break
      }
      ...
      cells.push(
        <td key={ds} colSpan={span}
          ...
          <div style={{padding:'4px 8px',display:'flex',alignItems:'flex-start',gap:4,minHeight:52}}>
            ...
            {!isVirtual && (
              <div style={{display:'flex',flexDirection:'column',gap:2,flexShrink:0,opacity:0}}
                className="task-handles"
                onMouseEnter={e=>e.currentTarget.style.opacity='1'}
                onMouseLeave={e=>e.currentTarget.style.opacity='0'}>
                <div title="Drag left edge" style={{cursor:'col-resize',fontSize:10,color:'rgba(255,255,255,.5)',padding:'0 2px'}}
                  onMouseDown={e=>startResize(e,member.name,startDs,'start')}>⋮</div>
                <div title="Copy to another person" style={{cursor:'grab',fontSize:10,color:'rgba(255,255,255,.4)',padding:'0 2px'}}
                  onMouseDown={e=>startCopy(e,member.name,startDs)}>⊕</div>
                <div title="Drag right edge" style={{cursor:'col-resize',fontSize:10,color:'rgba(255,255,255,.5)',padding:'0 2px'}}
                  onMouseDown={e=>startResize(e,member.name,startDs,'end')}>⋮</div>
              </div>
            )}
          </div>
        </td>
  ...
  return (
    <tr data-member-row={member.name}
      ...
      onDrop={onRowDrop}
      style={{outline: isDragTarget&&rowDragSrc ? '1px dashed #4f8ef7' : 'none'}}
      ...
      <td style={{background:'#1e2335',border:'1px solid #2a3050',padding:'6px 8px 6px 12px',verticalAlign:'middle'}}>
        <div style={{display:'flex',alignItems:'center',gap:6}}>
          ...
          <div style={{display:'flex',flexDirection:'column',gap:1,opacity:hovered?1:0,transition:'opacity .15s'}}>
            <button onClick={()=>onMoveRow(-1)} style={{background:'none',border:'none',cursor:'pointer',
              color:'#9aa3c2',padding:'1px 3px',fontSize:10,lineHeight:1}}>▲</button>
            <button onClick={()=>onMoveRow(1)} style={{background:'none',border:'none',cursor:'pointer',
              color:'#9aa3c2',padding:'1px 3px',fontSize:10,lineHeight:1}}>▼</button>
          </div>
        </div>
    ...
    <div>
      <AdminSection title="🏗 Active Projects" onAdd={()=>setProjectModal({})}>
        <table style={adminTableStyle}><thead><tr>
          ...
          <th style={adminTh}>Project Name</th><th style={adminTh}>Status</th><th style={adminTh}>Actions</th>
        </tr></thead><tbody>
          {projects.map(p=>(
  ...
  return (
    <AdminSection title="👥 Team Members" onAdd={()=>setMemberModal({})}>
      <table style={adminTableStyle}><thead><tr>
        ...
        <th style={adminTh}>Office</th><th style={adminTh}>Order</th><th style={adminTh}>Actions</th>
      </tr></thead><tbody>
        {teamMembers.map((m,i)=>(
          <tr key={m.id} style={{borderBottom:'1px solid #2a3050'}}>
            ...
            <td style={adminTd}>
              ...
              <button onClick={async()=>{await supabase.from('team_members').delete().eq('id',m.id);
...
function AssignModal({modal,onClose,projects,adminTasks,onSave,onClear,showToast}) {
  ...
  async function save() {
    ...
    await onSave(name,dateStr,isLeave?'':pid,label,fw,endDate,notes)
    onClose()
  }
  return (
...
function ProjectModal({item,onClose,onSave,projects,adminTasks}) {
  ...
  return (
    <Modal open onClose={onClose}>
      ...
      <label style={I.label}>Job #</label><input value={job} onChange={e=>setJob(e.target.value)} placeholder="e.g. 23-081" style={I.base} />
      <label style={I.label}>Project Name</label><input value={name} onChange={e=>setName(e.target.value)} placeholder="e.g. Waterfront" style={I.base} />
...
function AdminTaskModal({item,onClose,onSave,projects,adminTasks}) {
  ...
  return (
    <Modal open onClose={onClose}>
      ...
      <label style={I.label}>Task Name</label><input value={name} onChange={e=>setName(e.target.value)} style={I.base} />
...
function MemberModal({item,onClose,onSave,teamMembers}) {
  ...
  return (
    <Modal open onClose={onClose}>
      ...
      <label style={I.label}>Name</label><input value={name} onChange={e=>setName(e.target.value)} placeholder="Full name" style={I.base} />
      <label style={I.label}>Role</label><input value={role} onChange={e=>setRole(e.target.value)} placeholder="e.g. 3D Modeller" style={I.base} />
...
function LeaveModal({item,onClose,onSave,teamMembers,showToast}) {
  ...
  return (
    <Modal open onClose={onClose}>
      ...
      <div style={I.row}>
        <div><label style={I.label}>Start Date</label><input type="date" value={startDate} onChange={e=>setStartDate(e.target.value)} style={I.base} /></div>
        <div><label style={I.label}>End Date</label><input type="date" value={endDate} onChange={e=>setEndDate(e.target.value)} style={I.base} /></div>
      </div>
      ...
      <div style={{display:'flex',gap:8,marginTop:18,justifyContent:'flex-end'}}>
        ...
        <button onClick={()=>{
          ...
          let d=dates
          if(!d){const s=parseLocalDate(startDate),e=parseLocalDate(endDate),n=Math.round((e-s)/86400000)+1;d=`${fmtDisplay(s)} – ${fmtDisplay(e)}${n>1?' ('+n+' days)':''}`}
...
function PHModal({item,onClose,onSave,showToast}) {
  ...
  return (
    <Modal open onClose={onClose}>
      ...
      <div style={I.row}>
        <div><label style={I.label}>Date</label><input type="date" value={isoDate} onChange={e=>setIsoDate(e.target.value)} style={I.base} /></div>
        <div><label style={I.label}>End Date (optional)</label><input type="date" value={endIsoDate} onChange={e=>setEndIsoDate(e.target.value)} style={I.base} /></div>
      </div>
      <div style={{display:'flex',gap:8,marginTop:18,justifyContent:'flex-end'}}>
        <button onClick={onClose} style={btnBase}>Cancel</button>
        <button onClick={()=>{
